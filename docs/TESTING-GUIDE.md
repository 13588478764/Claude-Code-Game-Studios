...
每个系统都有对应的测试场景，可以直接运行进行深度测试：

| 测试场景 | 功能 | 测试重点 |
|----------|------|----------|
| `equipment_test.tscn` | 装备系统 | 装备槽位、品阶颜色、属性加成 |
| `battle_test.tscn` | 战斗系统 | 伤害计算、暴击、弱点克制 |
| `encounter_test.tscn` | 奇遇系统 | 触发概率、保底机制、奖励发放 |
| `quest_test.tscn` | 任务系统 | 任务接取、目标追踪、奖励发放 |
| `save_test.tscn` | 存档系统 | 保存/加载、数据持久化 |
| `world_test.tscn` | 世界系统 | 区域探索、快速旅行、境界限制 |
