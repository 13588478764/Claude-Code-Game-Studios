...
signal encounter_event_processed(
	encounter_type: String, encounter_id: String, reward_data: Dictionary
)
...
func _trigger_encounter(trigger_type: String) -> void:
	...
	if triggered:
		...
		print(
			(
				"[EncounterEventHandler] 奇遇未触发，连续失败次数: %d"
				% _encounter_trigger_manager.consecutive_failures
			)
		)
...
func _grant_encounter_rewards(encounter_type: String, encounter_id: String) -> Dictionary:
	...
	if _encounter_integration != null:
		var encounter_data = {"id": encounter_id, "type": encounter_type}
...
func _get_player_data_snapshot() -> Dictionary:
	...
	return {"level": level_value, "realm": realm_value, "attributes": _get_character_attributes()}
