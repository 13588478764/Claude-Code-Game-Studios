...
func _load_all_dialogues() -> void:
	...
	print(
		"[DialogueManager] Auto-loaded %d dialogue files from res://data/dialogues/" % loaded_count
	)
...
func _parse_dialogue_tree(data: Dictionary) -> DialogueData.DialogueTree:
	var tree := DialogueData.DialogueTree.new(data.get("id", ""), data.get("title", ""))
...
func _parse_dialogue_node(data: Dictionary) -> DialogueData.DialogueNode:
	var node := DialogueData.DialogueNode.new(
		data.get("id", ""), data.get("speaker", ""), data.get("text", "")
	)
...
func _parse_choice(data: Dictionary) -> DialogueData.Choice:
	var choice := DialogueData.Choice.new(
		data.get("id", ""), data.get("text", ""), data.get("next_node", "")
	)
	...
	match type_str:
		...
		"relationship", "relationship_check":
			condition = DialogueData.RelationshipCondition.new(
				data.get("target", ""), data.get("value", 0), data.get("operator", ">=")
			)
		# 道心值条件（支持 dao_heart 和 dao_heart_check）
		"dao_heart", "dao_heart_check":
			condition = DialogueData.DaoHeartCondition.new(
				data.get("value", 0), data.get("operator", ">=")
			)
		"realm_level":
			condition = DialogueData.RealmLevelCondition.new(data.get("value", 1))
		"quest_status":
			condition = DialogueData.QuestStatusCondition.new(
				data.get("target", ""), data.get("value", "completed")
			)
		"item_owned":
			condition = DialogueData.ItemOwnedCondition.new(
				data.get("target", ""), data.get("value", 1)
			)
		"flag":
			condition = DialogueData.FlagCondition.new(
				data.get("target", ""), data.get("value", true)
			)
	...
	match type_str:
		...
		"modify_relationship", "relationship_change":
			effect = DialogueData.ModifyRelationshipEffect.new(
				data.get("target", ""), data.get("value", 0), data.get("reason", "")
			)
		...
		"modify_dao_heart", "dao_heart_change":
			effect = DialogueData.ModifyDaoHeartEffect.new(
				data.get("value", 0), data.get("reason", "")
			)
		# 解锁任务（支持 unlock_quest 和 quest_trigger）
		"unlock_quest", "quest_trigger":
			effect = DialogueData.UnlockQuestEffect.new(data.get("target", ""))
		# 给予物品（支持 give_item / item_give / grant_item）
		"give_item", "item_give", "grant_item":
			effect = DialogueData.GiveItemEffect.new(
				data.get("target", data.get("item_id", "")), data.get("value", data.get("count", 1))
			)
		"give_exp", "grant_exp":
			effect = DialogueData.GiveExpEffect.new(data.get("value", 0))
		"grant_silver":
		...
		# 设置标志位
		"set_flag":
			effect = DialogueData.SetFlagEffect.new(data.get("target", ""), data.get("value", true))
		# 声望变化（新增）
		"reputation_change":
			effect = DialogueData.ChangeReputationEffect.new(
				data.get("target", ""), data.get("value", 0)
			)
		# 给予技能（新增）
		"skill_give":
			effect = DialogueData.GiveSkillEffect.new(data.get("target", ""), data.get("value", 1))
		# 消耗物品/灵气（新增）
		...
		# 时间消耗（新增）
		"time_cost":
			effect = DialogueData.ConsumeTimeEffect.new(data.get("value", 0))
		# 体力消耗（新增）
		"strength_cost":
			effect = DialogueData.ConsumeStrengthEffect.new(data.get("value", 0))
		# 地图标记（新增）
		"map_mark":
			effect = DialogueData.MapMarkEffect.new(data.get("target", ""), true)
		# 商店打开（新增）
		"shop_open":
			effect = DialogueData.OpenShopEffect.new(data.get("target", ""))
		# 功法解锁（新增）
		...
		# 被动技能给予
		"passive_give":
			effect = DialogueData.GiveSkillEffect.new(data.get("target", ""), 1)
		...
		"trigger_combat":
			effect = DialogueData.TriggerCombatEffect.new(
				data.get("target", ""), data.get("config", {}), data.get("callback_node", "")
			)
		# 播放过场动画（场景级操作，事务不回滚）
		"cutscene_play":
			effect = DialogueData.PlayCutsceneEffect.new(data.get("target", ""))
		_:
			effect = DialogueData.Effect.new()
...
## 开始对话（通过对话ID）
## start_node_id 可选：指定起始节点（用于战斗后跳转到回调分支），为空时用默认起始节点
func start_dialogue(dialogue_id: String, start_node_id: String = "") -> bool:
	if _is_in_dialogue:
	...
	if not start_node_id.is_empty():
		var jump_node = _current_dialogue.get_node(start_node_id)
		if jump_node != null:
			_current_node = jump_node
		else:
			push_warning("对话树 %s 不存在节点 %s，回退到起始节点" % [dialogue_id, start_node_id])
...
func queue_dialogue(dialogue_id: String, priority: int = 1) -> void:
	_dialogue_queue.append({"dialogue_id": dialogue_id, "priority": priority})
