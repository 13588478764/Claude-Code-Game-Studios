...
# 任务状态枚举
enum QuestStatus { LOCKED, AVAILABLE, ACTIVE, COMPLETED, FINISHED }  # 锁定：前置条件未满足  # 可接取：条件满足，玩家可接取  # 进行中：玩家已接取，正在追踪目标  # 已完成：目标达成，等待提交  # 已结束：奖励已领取，任务归档

# 任务类型枚举
enum QuestType { MAIN, SIDE, BOUNTY, ENCOUNTER }  # 主线任务  # 支线任务  # 悬赏任务  # 奇遇任务

# 目标类型枚举
enum ObjectiveType { TALK_TO_NPC, KILL_ENEMY, COLLECT_ITEM, GO_TO_LOCATION, USE_ITEM }  # 与NPC对话  # 击杀敌人  # 收集物品  # 到达指定位置  # 使用物品
...
class Objective:
	...
	func _init(
		p_type: ObjectiveType, p_target_id: String, p_target_count: int, p_description: String
	):
		type = p_type
...
# 注册任务定义
func register_quest(
	quest_id: String, title: String, description: String, quest_type: QuestType, npc_id: String = ""
) -> bool:
...
func update_quest_status(quest_id: String, new_status: QuestStatus) -> bool:
	var old_status: QuestStatus = QuestStatus.LOCKED

	# 未注册的任务直接拒绝，避免后续未定义状态比较
	if not active_quests.has(quest_id) and not quest_definitions.has(quest_id):
		push_warning("[QuestManager] 未注册的任务，无法更新状态: " + quest_id)
		return false
	...
	if old_status != new_status:
		emit_signal(
			...
			quest_id,
			QuestStatus.keys()[old_status],
			QuestStatus.keys()[new_status]
		)
...
func update_objective_progress(quest_id: String, objective_index: int, increment: int = 1) -> bool:
	...
	print(
		"Updated objective progress for quest ",
		quest_id,
		", objective ",
		objective_index,
		": ",
...
func save_quest_data() -> Dictionary:
	...
	for quest_id in active_quests:
		var quest = active_quests[quest_id]
		var quest_data = {"id": quest.id, "status": quest.status, "objectives": []}
		...
		for quest_id in data.active_quests:
			...
			if quest_definitions.has(quest_id):
				...
				var active_quest = Quest.new(
					...
					quest_template.description,
					quest_template.quest_type,
