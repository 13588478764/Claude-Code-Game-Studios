...
func validate_character_system():
	...
	# 测试境界数量
	# 注意：在character_system.gd中，境界定义为REALMS常量，不是实例属性
	# 我们需要通过其他方式验证境界数量
...
func validate_encounter_system():
	...
	# 测试触发概率（使用非MAP_MOVEMENT上下文避免冷却时间）
	var can_trigger = encounter_system.can_trigger_encounter(
		encounter_system.TriggerContext.COMBAT_VICTORY, 
		50
	)
	
	# 测试保底机制（使用非MAP_MOVEMENT上下文避免冷却时间）
	...
	# 测试实际触发奇遇
	var triggered_encounter = encounter_system.trigger_encounter(50, 10, 1, encounter_system.TriggerContext.COMBAT_VICTORY)
...
func validate_quest_system():
	...
	# 直接添加到进行中任务
	quest_system.active_quests["test_quest"] = test_quest
	
	# 验证任务是否正确添加
	if not quest_system.active_quests.has("test_quest"):
		print("❌ 任务系统添加任务失败")
		return false
	
	# 测试奖励发放
	quest_system.distribute_quest_rewards(test_quest)
