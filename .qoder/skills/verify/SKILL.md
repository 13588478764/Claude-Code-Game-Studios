...
1. 依次运行四条测试命令（每条可能需要 1-3 分钟，耐心等待完成）：
   ```bash
   godot --headless --script addons/gut/gut_cmdln.gd -gdir=tests/unit -ginclude_subdirs -glog=1
   godot --headless --script addons/gut/gut_cmdln.gd -gdir=tests/unit -ginclude_subdirs -gprefix= -gsuffix=_test.gd -glog=1
   godot --headless --script addons/gut/gut_cmdln.gd -gdir=tests/integration -ginclude_subdirs -glog=1
   godot --headless --script addons/gut/gut_cmdln.gd -gdir=tests/integration -ginclude_subdirs -gprefix= -gsuffix=_test.gd -glog=1
   ```
   注意：仓库有两套测试命名并存——`test_*.gd`（GUT 默认 prefix 发现）和 `*_test.gd`（大多数测试，需 `-gprefix= -gsuffix=_test.gd`）。只跑一条会漏掉一整套。
