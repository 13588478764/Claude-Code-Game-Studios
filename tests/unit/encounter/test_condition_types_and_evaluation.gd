...
func before_each():
	_evaluator = load("res://src/scripts/encounter/condition_evaluator.gd").new()
...
func test_four_condition_types_correctly_implemented():
	...
	var temporal_result = _evaluator.evaluate_temporal_environment_conditions(player_dict)
	...
	var character_result = _evaluator.evaluate_character_state_conditions(character_dict)
	...
	var progress_result = _evaluator.evaluate_progress_history_conditions(progress_dict)
	assert_true(progress_result, "进度历史条件评估应该通过")

	# 测试随机概率条件
	var random_result = _evaluator.evaluate_random_probability_conditions(50.0)
...
func test_condition_evaluation_logic_correct():
	# 创建测试条件组 - AND逻辑
	var and_group = _evaluator.ConditionGroup.new()
	and_group.logic_op = _evaluator.LogicOp.AND
	and_group.conditions = [
		create_test_condition(_evaluator.ConditionType.TEMPORAL_ENVIRONMENT, true),
		create_test_condition(_evaluator.ConditionType.CHARACTER_STATE, true),
		create_test_condition(_evaluator.ConditionType.PROGRESS_HISTORY, true)
	]

	var player_data = create_test_player_data()
	var progress_data = create_test_progress_data()
	var and_result = _evaluator.evaluate_condition_group(and_group, player_data, progress_data)
	assert_true(and_result, "AND逻辑应该返回true（所有条件都为true）")

	# 创建测试条件组 - OR逻辑
	var or_group = _evaluator.ConditionGroup.new()
	or_group.logic_op = _evaluator.LogicOp.OR
	or_group.conditions = [
		create_test_condition(_evaluator.ConditionType.TEMPORAL_ENVIRONMENT, false),
		create_test_condition(_evaluator.ConditionType.CHARACTER_STATE, true),
		create_test_condition(_evaluator.ConditionType.PROGRESS_HISTORY, false)
	]

	var or_result = _evaluator.evaluate_condition_group(or_group, player_data, progress_data)
...
func test_luck_correction_coefficient_correctly_applied():
	# 测试不同福缘值的修正系数
	var prob_low_luck = _evaluator.calculate_trigger_probability(0.05, 10.0)  # 10点福缘
	var prob_medium_luck = _evaluator.calculate_trigger_probability(0.05, 50.0)  # 50点福缘
	var prob_high_luck = _evaluator.calculate_trigger_probability(0.05, 90.0)  # 90点福缘
	var prob_very_high_luck = _evaluator.calculate_trigger_probability(0.05, 150.0)  # 150点福缘（应被限制在20%）
...
func test_condition_evaluation_result_accurate():
	...
	# 测试随机概率条件（使用高概率）
	var random_result = _evaluator.evaluate_random_probability_conditions(50.0, 0.5)  # 高概率测试
	assert_true(random_result or not random_result, "随机概率条件应该返回布尔值")


# 创建测试玩家数据
func create_test_player_data() -> Object:
	var player_data = _evaluator.PlayerData.new()
...
func create_test_progress_data() -> Object:
	var progress_data = _evaluator.ProgressData.new()
...
func create_test_condition(condition_type, is_met) -> Object:
	var condition = _evaluator.Condition.new()
