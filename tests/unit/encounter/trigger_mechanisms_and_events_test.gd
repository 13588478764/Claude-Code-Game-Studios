...
func test_event_listening_mechanism_normal():
	...
	# 使用 GUT 的信号监视功能
	watch_signals(trigger_manager)
	
	# 触发一个全局事件
	trigger_manager.handle_global_events("rest_started", {"time": "night"})
	
	# 检查信号是否被发出
	assert_signal_emitted(trigger_manager, "global_event_handled", "应该发出 global_event_handled 信号")
	assert_signal_emit_count(trigger_manager, "global_event_handled", 1, "应该发出 1 次 global_event_handled 信号")