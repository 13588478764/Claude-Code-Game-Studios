...
func before_each():
	_logic_manager = load("res://src/scripts/encounter/logic_tree_manager.gd").new()
	add_child_autofree(_logic_manager)


func after_each():
	if is_instance_valid(_logic_manager):
		_logic_manager.queue_free()
		_logic_manager = null
...
func test_logic_tree_structure_correctly_implemented():
	...
	# 构建逻辑树
	var tree_root = _logic_manager.build_logic_tree(test_tree_data)
	
	# 验证树结构
	assert_eq(tree_root.logic_op, _logic_manager.LogicOp.AND, "根节点应该是AND操作")
	...
	assert_eq(child_node.logic_op, _logic_manager.LogicOp.OR, "子节点应该是OR操作")
...
func test_weight_allocation_mechanism_normal():
	...
	# 计算调整后权重
	var weighted_results = _logic_manager.calculate_adjusted_weights(test_encounters)
...
func test_mutex_group_handling_correct():
	...
	# 处理互斥组
	_logic_manager.handle_mutex_groups(test_mutex_data)
	
	# 验证互斥组是否正确创建
	assert_eq(_logic_manager.mutex_groups.size(), 2, "应该创建2个互斥组")
	assert_true(_logic_manager.mutex_groups.has("group_1"), "应该存在group_1")
	assert_true(_logic_manager.mutex_groups.has("group_2"), "应该存在group_2")
	
	# 测试冲突检查 - 初始状态应该没有冲突
	var no_conflict_initially = not _logic_manager.check_mutex_conflicts("encounter_1", "group_1")
	assert_true(no_conflict_initially, "初始状态应该没有冲突")
	
	# 更新组状态
	_logic_manager.update_mutex_group_status("encounter_1", "group_1")
	
	# 检查冲突 - 更新后应该有冲突
	var has_conflict_after_update = _logic_manager.check_mutex_conflicts("encounter_2", "group_1")
...
func test_weight_redistribution_algorithm_accurate():
	...
	for i in range(total_trials):
		var selected = _logic_manager.weighted_random_selection(weighted_results)
